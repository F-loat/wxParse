<template>
  <image class="img" :mode="node.imageMode" :class="node.classStr" :style="node.styleStr" :data-src="node.attr.src" :src="node.attr.src" @tap="wxParseImgTap" @load="wxParseImgLoad" />
</template>
...
<script>
  export default {
    ...
    methods: {
      getSysWH: function() {
        var that = this;
        //获取当前设备屏幕宽度, 写在这里只是为了方便调试, 最好是写到 wxParse.vue 再传入
        wx.getSystemInfo({
      ...
      calMoreImageInfo: function(e, that) {
        ...
        //自适应设备屏幕宽度主要在这里
        //原版的代码是修改Appdata的值,达到图片宽度高度动态变化的小花
        ...
        that.node.styleStr =
          "height:" + recal.imageheight + "px; width:" + recal.imageWidth + "px";
